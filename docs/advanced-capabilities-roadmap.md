...
| MCP server for Secrets Broker | Later | `service-lasso/lasso-secretsbroker`; optional UI in `service-lasso/lasso-serviceadmin` | Policy assignment (#62), audit hardening (#63), telemetry (#64), events (#65), lockout (#66), local API auth, and metadata-only diagnostics | MCP must not expose reveal, export, credential sync, provider token, or raw audit search by default. Any management tool must require local API auth, policy allow, audit reason, and redaction guard tests. | After #62-#66, create a read-only MCP design issue for broker status, source capability/status, safe event queries, and redacted telemetry. Mutations remain out of scope until separate approval. |
| Secrets Sync, including GitHub sync | Later | `service-lasso/lasso-secretsbroker`; target-specific adapters may involve `service-lasso/service-lasso` for package/runtime integration | Provider/source registry, write-back policy, audit schema, events, lockout, provider capability model, operator confirmation flow, and GitHub credential reference handling | Sync can copy sensitive material into third-party systems. It must require explicit destination policy, dry-run, audit, idempotency, rollback guidance, and proof that provider credentials and synced values are never logged or rendered. GitHub sync must distinguish GitHub Actions secrets, Dependabot secrets, environments, and org/repo scopes. | Add a provider-neutral sync planning contract only after audit/events/lockout are available. The first implementation should be dry-run metadata for source ref, destination kind, destination scope, capability, policy result, and blocker state; no apply path. |
...
| Secrets Broker MCP | Broker policy, audit, telemetry, events, lockout, and local API auth are implemented and validated. |
