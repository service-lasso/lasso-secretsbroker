...
Explicit OTLP HTTP export action for the same sanitized metadata envelope exposed by `GET /v1/telemetry`.

The action is disabled by default and sends only when all of the following are configured:

- `SECRETSBROKER_OTEL_ENABLED=1`
- `SECRETSBROKER_OTEL_EXPORT_MODE=export`
- `OTEL_EXPORTER_OTLP_ENDPOINT` is an HTTP(S) endpoint

Optional `OTEL_EXPORTER_OTLP_HEADERS` are passed to the exporter when they use supported HTTP header names. Header names and values are never returned by the API.

Safe response fields report only mode, status, protocol, content type, signal count, exporter status code, whether endpoint/headers were configured, and non-return booleans for endpoint/header/body values. The response never returns OTLP endpoint values, OTLP headers, exported payload bodies, raw refs, secret values, provider credentials, tokens, cookies, private keys, recovery material, environment values, raw URL paths/query strings, raw request/response bodies, provider response bodies, or raw config values.

Unsupported endpoint schemes and malformed header shapes are blocked before sending.
